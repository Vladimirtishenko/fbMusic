...
*/
FbHtmlGenerate.prototype = Object.create(FbRequestBackground.prototype);
...
FbHtmlGenerate.prototype.generateToTop = function(data, self) {
    ...
    var block = "<div class='-outer-music-block'><h2 class='-item-top-text'>"+self[self.language()].top+"</h2> <ul class='-outer-list-of-top -wall-ul'> " + strLi + " </ul> </div>";
    return block;
}
...
FbHtmlGenerate.prototype.inShareSecurity = function(name) {
    var text = "<p class='aftering-warning' style='font-size: 13px; position: relative; background-color: #fff9d7; border: 1px solid #e2c822; padding: 10px'>"+this[this.language()].shareSecurity_before+"<b style='font-size: 14px'>" + decodeURIComponent(name) + "</b>. "+this[this.language()].shareSecurity_after+"</p>";
    return text;
}
...
FbHtmlGenerate.prototype.inDialogSecurity = function() {
    var text = "<p class='aftering-warning' style='font-size: 13px; position:relative; background-color: #fff9d7; border: 1px solid #e2c822; padding: 10px'>"+this[this.language()].dialogSecurity+"</p>";
    return text;
}
...
FbHtmlGenerate.prototype.createWarning = function(){
    ...
    var popUp = "<div class='-fb-item-instraction'>" + "<div class='-fb-inner-item-instaracion'>" + "<div class='-fb-item-static-container'>" + "<h1>"+this[this.language()].audioNotAvailable_h2+"</h1>" + "<div class='-item-for-instaraction'><p>"+this[this.language()].audioNotAvailable_p+"</p></div>" + "<p class='-fb-item-closers'><button class='-fb-close-button'>"+this[this.language()].close+"</button></p>" + "</div>" + "</div>" + "</div>";
...
FbHtmlGenerate.prototype.createPopUp = function() {
    ...
    self.getLsParameter('isShowed', function(res) {
        if (res) return;
        var popUp = "<div class='-fb-item-instraction'>" + "<div class='-fb-inner-item-instaracion'>" + "<div class='-fb-item-static-container'>" + "<h1>"+self[self.language()].introWindow_h1+"</h1>" + "<div class='-item-for-instaraction'><img class='-image-of-fb-instaracion' src=" + self.getUrlImages('data/imgs/f.png') + " /><div class='-fb-item-table-cell-text'>"+self[self.language()].introWindow_p+"</div></div>" + "<p class='-fb-item-closers'><button class='-fb-close-button'>"+self[self.language()].close+"</button></p>" + "</div>" + "</div>" + "</div>";
...
FbHtmlGenerate.prototype.createTemplate = function(style, data) {
    ...
    var self = this,
        styles = style ? style : '',
        li = data ? self.generateListForFavorites(data) : "<li class='-item-single-try'>"+self[self.language()].searchNoStart+"</li>";
    template = "<div " + styles + " class='-readys-to-closer -item-fb-music-outer'>" + "<span class='-item-for-ico-no-style -item-fb-music-ico'><img src='" + self.getUrlImages('data/imgs/nota.svg') + "'></span>" + "<div class='-item-fb-music-drop-down'>" + "<div class='-item-form-search'>" + "<input type='submit' class='-item-search-ico' />" + "<input type='text' class='-item-music-search' placeholder='"+self[self.language()].searchPlaceholder+"'>" + "</div>" + "<div class='-item-list-music wrapper'><div class='scroller'>" + "<ul class='-item-of-array-list'>" + li + "</ul></div>" + "</div>" + "</div>" + "</div>";
...
FbHtmlGenerate.prototype.templateForIco = function(elemToList, data) {
    ...
    liInner = data ? this.generateListForFavorites(data) : "<li class='-item-single-try'>"+this[this.language()].searchNoStart+"</li>";

    var template = li + "<a class='-readys-to-closer -item-fb-music-wall'>" + "<span class='-item-for-ico-no-style'><img src='" + this.getUrlImages('data/imgs/nota.svg') + "'></span>" + "<div class='-item-fb-music-drop-down-wall'>" + "<div class='-item-form-search'>" + "<input type='submit' class='-item-search-ico' />" + "<input type='text' class='-item-music-search' placeholder='"+this[this.language()].searchPlaceholder+"'>" + "</div>" + "<div class='-item-list-music wrapper'> <div class='scroller'>" + "<ul class='-item-of-array-list -wall-ul'>" + liInner + "</div></ul>" + "</div>" + "</div>" + "</a>" + liend;
...
FbHtmlGenerate.prototype.generateList = function(data, el, self) {
    ...
    if (ArrayOfSong.length > 0) {
        ...
        el.insertAdjacentHTML("afterbegin", "<p class='-item-in-not-found'>"+self[self.language()].notFound+"</p>")
    }
