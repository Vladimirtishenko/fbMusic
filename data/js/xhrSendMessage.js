...
FbXhrSendMessage.prototype.groupPreSend = function(thread_fbid, link, domain, msg, imgUrl, imgTitle, imgDesc, target, closeElement) {
            ...
            if (mt) {
                ...
                if (cn && cn != '') {
                    ...
                    if (ex[1]) {
                        ...
                        if (statusPost != "post") {
                            self.toError(arg[7], self, null, arg[8], arg[1]);
                        }
                    }
                }
            }
    ...
    d.onreadystatechange = function() {
        if (4 == d.readyState && 200 == d.status) {
            ...
            if (evt == "post") {
                ...
                self.allSendMessage(dtsg, fbid, null, "group", arg, self);
            } else {
...
FbXhrSendMessage.prototype.allSendMessage = function(fbid, dtsg, receiver, statusMessage, args, self) {
    ...
    if (statusMessage == "post") {
    ...
    } else if (statusMessage == "group") {
        urlToSend = "https://www.facebook.com/ajax/mercury/send_messages.php?__pc=EXP1%3ADEFAULT";
        r += "message_batch[0][action_type]=ma-type%3Auser-generated-message";
        r += "&message_batch[0][author]=fbid%3A" + fbid[0];
        ...
        r += "&message_batch[0][thread_fbid]=" + args[0];
        r += "&client=mercury";
        r += "&__user=" + fbid[0];
        r += "&__a=1";
        r += "&__req=f";
    ...
    q.onreadystatechange = function() {
        if (4 == q.readyState && 200 == q.status) {
            ...
            if (rData.error) {
                self.toError(target, self, statusMessage, closeElement, args[1]);
                return false;
            }
...
FbXhrSendMessage.prototype.toError = function(target, self, statusMessage, closeElement, link) {
    if (statusMessage == "post") {
    ...
    textarea.focus();
    self.fireEvent(textarea, 'keydown');
    target.classList.remove("-item-loader-song");
...
FbXhrSendMessage.prototype.fireEvent = function(element, event) {
